...
public class SnakeGame extends JFrame {
			...
			public void keyPressed(KeyEvent e) {
				...
				case KeyEvent.VK_ENTER:
					if(isNewGame || isGameOver) {
						 ...
						 * if so set high score to current score
						 */
						if (score > highScore) {
							highScore = score; // set highscore equal to score
							/*
							 * Write highscore to external file
							 */
							try(PrintWriter writer = new PrintWriter("HighScore.txt")) { // write highscore to file
								writer.println(highScore);
								writer.close();
							} catch (FileNotFoundException ex) {
								// nothing needed here since we handled this on game start / reset
							}
						} 
						resetGame();
					}
	...
	private void resetGame() {
		...
		this.fruitsEaten = 0;
		/*
		 * Read highscores from external file when game is started
		 * 
		 */
		File inputFile = new File("HighScore.txt");
			try (Scanner fileReader = new Scanner(inputFile); // read in source
				...
				if (fileReader.hasNextLine()) { // only execute while next line exists in input file this prevents end
					highScore = Integer.parseInt(fileReader.nextLine());// get line from file , parse int , and set parsed int  to high score
				}
			} catch (FileNotFoundException ex) { // if file is not found create file
				try(PrintWriter writer = new PrintWriter("HighScore.txt")) { // create new file
					writer.println(0); // write 0 to file
					writer.close();
